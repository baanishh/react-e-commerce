  ...
  return (
    ...
    <div data-aos='zoom-in' className='flex justify-center'>
      <ul className='hidden sm:flex items-center gap-4'>
        {Menu.map(item=>(
          <li key={item.id}>
            <a href={item.link} className='px-4 inline-block hover:text-primary' >{item.name}</a>
          </li>
        ))}
     ...
     {/* simple drop-down menu  */}
        <li className='group relative cursor-pointer'>
          <a href="#" className='flex items-center gap-[2px] py-2'>
            Trending Products
            <span><FaCaretDown className='transition-all duratio-200 group-hover:rotate-180' /></span>
          </a>
          <div className='absolute z-[999px] hidden group-hover:block w-[150px] rounded-md bg-white p-2 text-black shadow-md'>
            <ul>
              {DropDownLinks.map(items=>(
                <li key={items.id}>
                  <a href={items.link} className='inline-block w-full rounded-md p-2 hover:bg-primary/20'>{items.name}</a>
                </li>
              ))}
            </ul>
          </div>
